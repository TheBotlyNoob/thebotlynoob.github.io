<script lang="ts">
  ...
  const load = async () =>
    await fetch(
      ...
      }&target[]=${post.path}&target[]=${post.path}/`
    )
      .then(res => res.json())
...
<div class="flex flex-col gap-8">
  ...
  {#if config?.form === true}
    <form id="webmention-form" method="post" action="https://webmention.io/{config.username}/webmention">
      <input type="hidden" name="target" value={post.path} />
        ...
        {#if config?.commentParade === true}
          <span class="label-text-alt text-right">
            or <a
              ...
              href="https://quill.p3k.io/?dontask=1&me=https://commentpara.de/&reply={encodeURI(post.path)}">
              comment anonymously
            </a>
          </span>
        {/if}
