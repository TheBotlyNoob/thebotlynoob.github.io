...
$(document).ready(async () => {
  var wrapElements = async () => {
    ...
    $("[class$='-icon']").each(function () {
      a = $(this).attr("class").split(" ").pop().replace("-icon", "");
      $(this).prepend(`<i class="icon fa fa-${a}"></i>`);
    });
  };
  ...
  var setThings = {
    GitHub: async () => {
      ...
      data = await fetch("https://api.github.com/users/TheBotlyNoob/repos").then(response => response.json()).then(data => {
    ...
    YouTube: async () => {
      data = await fetch("https://www.googleapis.com/youtube/v3/search?key=AIzaSyCyQEWcesl042F-EvVZsVWOh5Mw2ABId64&channelId=UCBpQy7-ZNMHhRBQ0gcHGcBw&part=snippet,id&order=date&maxResults=20").then(response => response.json()).then(data => {
        return JSON.parse(JSON.stringify(data));
      }).catch(error => {
        console.error(error);
      });

      for (i = 0; i < ((_data$items = data.items) === null || _data$items === void 0 ? void 0 : _data$items.length) - 1
    ...
    Cursors: async () => {
      ...
      a = async (selector, aniUrl) => {
        response = await fetch(aniUrl);
